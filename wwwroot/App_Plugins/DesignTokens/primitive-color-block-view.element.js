...
class MyBaseColorBlockView extends HTMLElement {
  ...
  render() {
    ...
    const content = `
      <span class="swatch" style="background:${this.escapeAttr(color)}"></span>
    ...
    this.shadowRoot.innerHTML = `
      ...
      ${editPath
        ? `<a class="primitive-colour" href="${this.escapeAttr(editPath)}">${content}</a>`
        : `<div class="primitive-colour">${content}</div>`}
    `;
  }
...
if (!customElements.get("my-base-color-block-view")) {
  customElements.define("my-base-color-block-view", MyBaseColorBlockView);
}
