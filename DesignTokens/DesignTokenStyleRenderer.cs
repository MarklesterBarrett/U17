...
public sealed class DesignTokenStyleRenderer : IDesignTokenStyleRenderer
{
    ...
    public ElementStyleOverrides GetElementStyleOverrides(IPublishedElement? settings)
    {
        ...
        var colorTokens = tokenSet.Colors.Select(x => x.Alias).ToHashSet(StringComparer.OrdinalIgnoreCase);
    ...
    private string? ResolveDeclaration(
        ...
        string? settingValue,
        IReadOnlySet<string> colorTokens,
    ...
    private static string? ResolveTokenValue(IReadOnlySet<string> tokens, string tokenAlias)
    {
